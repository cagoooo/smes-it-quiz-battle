const characters = [
  { id: 'coder', icon: '💻', image: 'assets/characters/coder.png', name: '程式勇者', skill: '演算法導航', color: '#52e6ff', ultimates:[{ name:'迴圈風暴', damage:34, color:'#52e6ff' }, { name:'演算法連鎖', damage:38, color:'#7af4ff' }, { name:'零錯誤雷擊', damage:42, color:'#b6fbff' }] },
  { id: 'guardian', icon: '🛡️', image: 'assets/characters/guardian.png', name: '網安守護者', skill: '防毒護盾', color: '#62f7bb', ultimates:[{ name:'防毒結界', damage:34, color:'#62f7bb' }, { name:'零信任護盾', damage:38, color:'#9affd8' }, { name:'資安天網', damage:42, color:'#d0ffeb' }] },
  { id: 'data', icon: '📊', image: 'assets/characters/data.png', name: '資料魔法師', skill: '圖表洞察', color: '#c5a5ff', ultimates:[{ name:'資料流星雨', damage:34, color:'#c5a5ff' }, { name:'圖表預言', damage:38, color:'#dfcaff' }, { name:'洞察超新星', damage:42, color:'#f0e7ff' }] },
  { id: 'creator', icon: '🎨', image: 'assets/characters/creator.png', name: '創意設計師', skill: '媒體創作', color: '#ffbd7a', ultimates:[{ name:'像素彩虹炮', damage:34, color:'#ffbd7a' }, { name:'靈感大爆發', damage:38, color:'#ffd6a9' }, { name:'創意銀河', damage:42, color:'#fff0dc' }] },
  { id: 'ai-explorer', icon: '🤖', image: 'assets/characters/ai-explorer.png', name: 'AI 探險家', skill: '模型導航', color: '#7fbaff', ultimates:[{ name:'神經網路脈衝', damage:34, color:'#7fbaff' }, { name:'智慧星圖', damage:38, color:'#a8d1ff' }, { name:'未來預測光束', damage:42, color:'#e0efff' }] },
  { id: 'green-engineer', icon: '🌱', image: 'assets/characters/green-engineer.png', name: '綠能工程師', skill: '永續雲端', color: '#91df70', ultimates:[{ name:'太陽能聚焦', damage:34, color:'#91df70' }, { name:'綠電旋風', damage:38, color:'#b5f79b' }, { name:'永續地球脈動', damage:42, color:'#e0ffd1' }] },
];

const cpuEnemies = [
  { id:'firewall', name:'防火牆怪', icon:'🧠', image:'assets/characters/firewall.png', ultimates:[{ name:'封包封鎖', damage:32, color:'#ff826f' }, { name:'熔岩防線', damage:36, color:'#ffb06c' }] },
  { id:'noise-beast', name:'雜訊干擾獸', icon:'📡', image:'assets/characters/noise-beast.png', ultimates:[{ name:'雜訊震盪', damage:32, color:'#e387ff' }, { name:'頻率風暴', damage:36, color:'#ffb0ee' }] },
  { id:'bug-phantom', name:'漏洞幻影', icon:'🧩', image:'assets/characters/bug-phantom.png', ultimates:[{ name:'錯誤裂縫', damage:32, color:'#7b9cff' }, { name:'藍屏幻象', damage:36, color:'#b4c7ff' }] },
];
...
function createFighter(name, icon, isCpu=false, image='', ultimates=[]) { return { name, icon, image, health:100, meter:0, streak:0, isCpu, ultimate:random(ultimates.length ? ultimates : [{ name:'同步衝擊', damage:30, color:'#ffe479' }]) }; }
...
function setupBattle() {
  state.p1 = createFighter(state.character.name, state.character.icon, false, state.character.image, state.character.ultimates);
  const enemy = random(cpuEnemies);
  state.p2 = state.mode === 'solo' ? createFighter(enemy.name, enemy.icon, true, enemy.image, enemy.ultimates) : createFighter('玩家 2', '👾');
...
function moveName(actor, level) { return level==='light'?'掃描衝擊':level==='heavy'?'除錯連發':actor.ultimate.name; }
...
function attack(actor,defender,damage,level) { const source=actor===state.p1?$('fighter-p1'):$('fighter-p2'), target=defender===state.p1?$('fighter-p1'):$('fighter-p2'); focusBattleScene(40); source.classList.remove('attack','ultimate'); void source.offsetWidth; source.classList.add(level==='ultimate'?'ultimate':'attack'); target.classList.remove('hit'); void target.offsetWidth; target.classList.add('hit'); defender.health=Math.max(0,defender.health-damage); const blast=document.createElement('i'); blast.className=`blast ${level==='heavy'?'heavy':''} ${level==='ultimate'?'ultimate':''}`; if(level==='ultimate')blast.style.setProperty('--blast-color',actor.ultimate.color); $('battle-fx').append(blast); window.setTimeout(()=>blast.remove(),700); if(actor.streak>=2){const combo=$('combo');combo.textContent=`${actor.streak} HIT COMBO!`;combo.classList.remove('hidden');window.setTimeout(()=>combo.classList.add('hidden'),800);} if(level==='ultimate') playMusicStinger('ultimate'); playTone(true,level); }
function cpuMove(){ if(!state.running||state.turn!=='p2')return; const actor=state.p2, level=actor.meter>=100&&Math.random()>.55?'ultimate':Math.random()>.52?'heavy':'light'; const q=getQuestion(level); const correct=Math.random()<(level==='light'?.78:level==='heavy'?.62:.5); state.current={level,q}; if(correct){actor.streak++;state.p1.streak=0;actor.meter=level==='ultimate'?0:Math.min(100,actor.meter+(level==='light'?34:48));$('feedback').textContent=`AI 找到正解！${actor.name} 發動 ${moveName(actor,level)}！`; $('feedback').className='feedback bad'; attack(actor,state.p1,moveDamage(actor,level),level);}else{actor.streak=0;$('feedback').textContent='AI 的資料判讀失誤，這回合沒有命中！';$('feedback').className='feedback good';playTone(false);} updateUI();window.setTimeout(()=>{if(!state.running)return;if(state.p1.health<=0)finish('p2');else setTurn('p1');},1450); }
