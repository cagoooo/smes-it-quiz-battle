const characters = [
  { id: 'coder', icon: '💻', image: 'assets/characters/coder.png', name: '程式勇者', skill: '演算法導航', color: '#52e6ff', moves:{light:'位元突擊',heavy:'除錯連發'}, ultimates:[{ name:'迴圈風暴', damage:34, color:'#52e6ff' }, { name:'演算法連鎖', damage:38, color:'#7af4ff' }, { name:'零錯誤雷擊', damage:42, color:'#b6fbff' }] },
  { id: 'guardian', icon: '🛡️', image: 'assets/characters/guardian.png', name: '網安守護者', skill: '防毒護盾', color: '#62f7bb', moves:{light:'護盾衝刺',heavy:'掃毒彈幕'}, ultimates:[{ name:'防毒結界', damage:34, color:'#62f7bb' }, { name:'零信任護盾', damage:38, color:'#9affd8' }, { name:'資安天網', damage:42, color:'#d0ffeb' }] },
  { id: 'data', icon: '📊', image: 'assets/characters/data.png', name: '資料魔法師', skill: '圖表洞察', color: '#c5a5ff', moves:{light:'資料切片',heavy:'圖表彈幕'}, ultimates:[{ name:'資料流星雨', damage:34, color:'#c5a5ff' }, { name:'圖表預言', damage:38, color:'#dfcaff' }, { name:'洞察超新星', damage:42, color:'#f0e7ff' }] },
  { id: 'creator', icon: '🎨', image: 'assets/characters/creator.png', name: '創意設計師', skill: '媒體創作', color: '#ffbd7a', moves:{light:'色票飛刃',heavy:'靈感連描'}, ultimates:[{ name:'像素彩虹炮', damage:34, color:'#ffbd7a' }, { name:'靈感大爆發', damage:38, color:'#ffd6a9' }, { name:'創意銀河', damage:42, color:'#fff0dc' }] },
  { id: 'ai-explorer', icon: '🤖', image: 'assets/characters/ai-explorer.png', name: 'AI 探險家', skill: '模型導航', color: '#7fbaff', moves:{light:'推論脈衝',heavy:'特徵鎖定'}, ultimates:[{ name:'神經網路脈衝', damage:34, color:'#7fbaff' }, { name:'智慧星圖', damage:38, color:'#a8d1ff' }, { name:'未來預測光束', damage:42, color:'#e0efff' }] },
  { id: 'green-engineer', icon: '🌱', image: 'assets/characters/green-engineer.png', name: '綠能工程師', skill: '永續雲端', color: '#91df70', moves:{light:'葉片迴旋',heavy:'風能推進'}, ultimates:[{ name:'太陽能聚焦', damage:34, color:'#91df70' }, { name:'綠電旋風', damage:38, color:'#b5f79b' }, { name:'永續地球脈動', damage:42, color:'#e0ffd1' }] },
  { id: 'robotics-ace', icon: '🦾', image: 'assets/characters/robotics-ace.png', name: '機器人操控師', skill: '機甲調校', color: '#ff7fc8', moves:{light:'伺服突進',heavy:'機械臂連擊'}, ultimates:[{ name:'鋼鐵流星拳', damage:35, color:'#ff7fc8' }, { name:'機甲零距離砲', damage:39, color:'#ffa8dd' }, { name:'自動導航終結', damage:43, color:'#ffe0f3' }] },
  { id: 'cloud-ranger', icon: '☁️', image: 'assets/characters/cloud-ranger.png', name: '雲端航海家', skill: '資料同步', color: '#6ee6e8', moves:{light:'雲端滑翔',heavy:'同步雷達'}, ultimates:[{ name:'備份星雲', damage:35, color:'#6ee6e8' }, { name:'伺服器流星雨', damage:39, color:'#9df9fb' }, { name:'全域同步風暴', damage:43, color:'#ddffff' }] },
];
...
const skillSet = (quick, quickDamage, heavy, heavyDamage, tactical, tacticalDamage) => [
  { id:'quick', name:quick, damage:quickDamage, meterGain:28, questionLevel:'light', label:'基礎題', icon:'⌁' },
...
const cpuProfiles = { defender:{accuracy:.68,quick:.55,heavy:.3,tactical:.15}, burst:{accuracy:.61,quick:.22,heavy:.42,tactical:.36}, combo:{accuracy:.65,quick:.36,heavy:.35,tactical:.29}, disrupt:{accuracy:.58,quick:.45,heavy:.22,tactical:.33} };
const skillSets = {
...
const state = { mode:'solo', unit:'mixed', difficulty:'standard', character:characters[0], turn:'p1', running:false, resolving:false, time:75, timerId:null, questionTimerId:null, current:null, used:[], sound:true, correct:0, bestStreak:0, round:1, p1:null, p2:null };
const $ = (id) => document.getElementById(id);
const random = (items) => items[Math.floor(Math.random() * items.length)];

function renderRoster() { $('roster').innerHTML = characters.map(hero => `<button class="hero-card ${hero.id===state.character.id?'selected':''}" data-hero="${hero.id}" type="button"><span class="hero-portrait" style="--hero-color:${hero.color}">${hero.image ? `<img src="${hero.image}" alt="${hero.name}角色圖">` : `<i aria-hidden="true">${hero.icon}</i>`}</span><span class="hero-content"><b>${hero.name}</b><small>${hero.skill}</small></span><em>選擇</em></button>`).join(''); }
...
function createFighter(profile, isCpu=false) { const cpuStyle={firewall:'defender','noise-beast':'disrupt','bug-phantom':'combo','phishing-siren':'disrupt','cache-golem':'defender','glitch-dragon':'burst','malware-mimic':'combo','bot-swarm':'burst'}[profile.id] || 'defender'; return { id:profile.id, name:profile.name, icon:profile.icon, image:profile.image, health:100, meter:0, streak:0, isCpu, cpuProfile:cpuProfiles[cpuStyle], skills:skillSets[profile.id] || skillSet('掃描衝擊',8,'除錯連發',16,'同步戰術',22), ultimate:random(profile.ultimates?.length ? profile.ultimates : [{ name:'同步衝擊', damage:30, color:'#ffe479' }]) }; }
...
function setupBattle() {
  ...
  state.p2 = state.mode === 'solo' ? createFighter(enemy, true) : createFighter({ id:'duel', name:'玩家 2', icon:'👾', image:'', ultimates:[] });
...
function setTurn(turn) { if(!state.running)return; state.resolving=false; state.turn=turn; state.current=null; const actor=state[turn]; $('turn-title').textContent=actor.isCpu?'駕駛艙 AI 正在分析資料…':`${actor.name} 的回合：選擇技能`; document.querySelector('.turn-strip').classList.toggle('cpu',actor.isCpu); $('move-deck').classList.toggle('hidden',actor.isCpu); if(!actor.isCpu)renderMoveDeck(actor); $('question-card').classList.add('hidden'); $('feedback').textContent=actor.isCpu?'AI 正在挑選它的招式…':'選擇四種專屬技能之一，答對就能發動攻擊！'; $('feedback').className='feedback'; updateUI(); if(actor.isCpu) window.setTimeout(cpuMove,difficultyModes[state.difficulty].delay); }
...
function playUltimateCombo(actor, defender, damage) {
  ...
  for(let index=0;index<hits;index++){
    ...
    window.setTimeout(()=>{
      ...
      defender.health=Math.max(0,defender.health-chunks[index]); updateUI();
...
function chooseMove(skillId) { if(!state.running||state.resolving||state.turn!=='p1' && state.mode==='solo')return; const actor=state[state.turn], skill=getSkill(actor,skillId); if(skillId==='ultimate'&&actor.meter<100)return; state.current={skill,q:getQuestion(skill.questionLevel)}; $('question-level').textContent=`${skill.label} · ${skill.name} · 傷害 ${skill.damage}`; $('question-count').textContent=`已答對 ${state.correct} 題`; $('question-text').textContent=state.current.q.q; $('answers').innerHTML=state.current.q.a.map((answer,index)=>`<button class="answer" data-answer="${index}" type="button"><b>${'ABCD'[index]}.</b> ${answer}</button>`).join(''); $('explain').textContent=state.difficulty==='practice'?difficultyModes.practice.hint:''; $('question-card').classList.remove('hidden'); $('move-deck').classList.add('hidden'); $('feedback').textContent='題目已就位，選出最合適的答案！'; startQuestionTimer(skill); focusQuestionCard(100); }
function answer(index,timedOut=false) { if(!state.current||!state.running)return; clearQuestionTimer(); const {q,skill}=state.current; document.querySelectorAll('.answer').forEach(btn=>btn.disabled=true); const correct=!timedOut&&index===q.correct, selected=document.querySelector(`.answer[data-answer="${index}"]`); if(selected)selected.classList.add(correct?'correct':'wrong'); if(!correct)document.querySelector(`.answer[data-answer="${q.correct}"]`).classList.add('correct'); $('explain').textContent=`💡 ${q.tip}`; const actor=state[state.turn], defender=state.turn==='p1'?state.p2:state.p1; let attackRecovery=1600; if(correct){state.correct++; actor.streak++; defender.streak=0; state.bestStreak=Math.max(state.bestStreak,actor.streak); const damage=moveDamage(actor,skill.id); actor.meter=skill.id==='ultimate'?0:Math.min(100,actor.meter+skill.meterGain); $('feedback').textContent=`答對！${actor.name} 發動 ${moveName(actor,skill.id)}！`; $('feedback').className='feedback good'; attackRecovery=attack(actor,defender,damage,skill.id); } else { actor.streak=0; actor.meter=Math.max(0,actor.meter-8); $('feedback').textContent=timedOut?'時間到！這題先記下來。':'這題先記下來！攻擊沒有命中。'; $('feedback').className='feedback bad'; playTone(false); }
  updateUI(); const recovery=correct ? attackRecovery : 1600; window.setTimeout(()=>{ if(!state.running)return; if(defender.health<=0)finish(state.turn); else setTurn(state.turn==='p1'?'p2':'p1'); },recovery);
}
function attack(actor,defender,damage,skillId) { const source=actor===state.p1?$('fighter-p1'):$('fighter-p2'), target=defender===state.p1?$('fighter-p1'):$('fighter-p2'); focusBattleScene(40); source.classList.remove('attack','ultimate'); void source.offsetWidth; source.classList.add(skillId==='ultimate'?'ultimate':'attack'); const blast=document.createElement('i'); blast.className=`blast ${skillId==='heavy'||skillId==='tactical'?'heavy':''} ${skillId==='ultimate'?'ultimate':''}`; if(skillId==='ultimate'){state.resolving=true;$('feedback').textContent='連招施放中，請觀察每一段命中！';blast.style.setProperty('--blast-color',actor.ultimate.color); const duration=playUltimateCombo(actor,defender,damage); $('battle-fx').append(blast); window.setTimeout(()=>blast.remove(),duration); playMusicStinger('ultimate'); playTone(true,skillId); return duration; } target.classList.remove('hit'); void target.offsetWidth; target.classList.add('hit'); defender.health=Math.max(0,defender.health-damage); $('battle-fx').append(blast); window.setTimeout(()=>blast.remove(),700); if(actor.streak>=2){const combo=$('combo');combo.textContent=`${actor.streak} HIT COMBO!`;combo.classList.remove('hidden');window.setTimeout(()=>combo.classList.add('hidden'),800);} playTone(true,skillId); return 1600; }
function cpuMove(){ if(!state.running||state.turn!=='p2')return; const actor=state.p2, mode=difficultyModes[state.difficulty], profile=actor.cpuProfile, weighted=actor.skills.flatMap(skill=>Array(Math.max(1,Math.round(profile[skill.id]*10))).fill(skill)), skill=actor.meter>=100&&Math.random()<mode.ultimateChance?getSkill(actor,'ultimate'):random(weighted), q=getQuestion(skill.questionLevel), base=skill.id==='quick'?.78:skill.id==='heavy'?.62:.52, correct=Math.random()<Math.min(.9,Math.max(.2,base+profile.accuracy-.65+mode.accuracy)); state.current={skill,q}; let recovery=1450; if(correct){actor.streak++;state.p1.streak=0;actor.meter=skill.id==='ultimate'?0:Math.min(100,actor.meter+skill.meterGain);$('feedback').textContent=`${mode.label} AI 找到正解！${actor.name} 發動 ${moveName(actor,skill.id)}！`; $('feedback').className='feedback bad'; recovery=attack(actor,state.p1,moveDamage(actor,skill.id),skill.id);}else{actor.streak=0;$('feedback').textContent='AI 的資料判讀失誤，這回合沒有命中！';$('feedback').className='feedback good';playTone(false);} updateUI();window.setTimeout(()=>{if(!state.running)return;if(state.p1.health<=0)finish('p2');else setTurn('p1');},Math.max(recovery,mode.delay)); }
