...
test('本機最佳紀錄使用 localStorage，讀寫皆需容錯（壞資料/無痕模式不可讓遊戲當機）', () => {
  ...
  assert.match(game, /function saveRecords\(records\)\{\s*try\s*\{/, 'saveRecords 寫入 localStorage 必須包在 try/catch 內');
  assert.match(game, /function renderBestRecord\(/);
  assert.match(game, /function updateRecords\(/);
  assert.match(game, /updateRecords\(\);\s*\$\('result'\)\.classList\.remove\('hidden'\)/, '每次結算都要更新本機最佳紀錄');
...
test('答對音效隨連擊數微幅升高音調，且大絕能量剛滿時會播放專屬提示音', () => {
  ...
  assert.match(game, /const meterBefore=actor\.meter;\s*actor\.meter=skill\.id==='ultimate'\?0:Math\.min\(100,actor\.meter\+skill\.meterGain\);\s*if\(actor\.meter>=100&&meterBefore<100\)playUltimateReadyChime\(\)/, '玩家答對後能量剛滿 100 時（而非每次都播）才觸發大絕提示音');
  assert.match(game, /const meterBefore=actor\.meter;\s*actor\.meter=skill\.id==='ultimate'\?0:Math\.min\(100,actor\.meter\+skill\.meterGain\);\s*if\(actor\.meter>=100&&meterBefore<100\)playUltimateReadyChime\(\)/, 'CPU 答對後能量剛滿 100 時也要觸發大絕提示音，行為需與玩家一致');
});
...
test('大廳歷史最佳紀錄提供重置入口，且需經確認才會清除', () => {
  ...
  assert.match(game, /function resetRecords\(\)\{\s*if\(!window\.confirm\(/, '重置前必須先跳出確認，避免誤觸就清空紀錄');
...
test('成就徽章系統：5 種固定徽章、不重複解鎖、資料存在 records.badges', () => {
  ...
  assert.equal(BADGES.length, 6, `徽章清單應固定 6 種，包含魔王挑戰徽章（目前 ${BADGES.length} 種）`);
...
test('CPU 敵人專屬戰鬥音樂：輪到 CPU 出招時切到牠的音軌，換回玩家回合時切回', () => {
  ...
  assert.match(game, /const cpuScene=cpuBattleSceneKey\(actor\.id\);\s*if\(cpuScene\)playMusicScene\(cpuScene\);/, 'setTurn 需依 CPU 是否有專屬音軌切換，玩家回合則切回 battleSceneKey()');
...
test('學生個人錯題報告與列印', () => {
  assert.match(consoleHtml, /id="student-report-modal"/);
  assert.match(consoleHtml, /id="print-student-report-btn"/);
  assert.match(consoleHtml, /window\.showStudentReport\s*=\s*function/);
});

test('多班級/組別對抗賽設定與同步', () => {
  assert.match(consoleHtml, /id="team-mode-select"/);
...
test('對戰表一鍵隨機與排行前 8 名生成', () => {
  assert.match(consoleHtml, /id="generate-rank-bracket-btn"/);
  assert.match(consoleHtml, /\$\('generate-rank-bracket-btn'\)\.addEventListener\('click'/);
});
...
test('Boss 戰模式支援 (全班 Raid 與單人魔王挑戰)', () => {
  ...
  assert.match(consoleHtml, /id="tab-boss"/);
  assert.match(consoleHtml, /id="boss-giant-image"/);
  assert.match(consoleHtml, /id="boss-hp-fill"/);
  ...
  assert.match(consoleHtml, /class="btn btn-tactical-ready"/);
...
test('Esports 級賽事動態播報跑馬燈與進行曲', () => {
  const consoleHtml = fs.readFileSync(path.join(root, 'console.html'), 'utf8');
  assert.match(consoleHtml, /id="announcer-ticker-container"/);
  assert.match(consoleHtml, /function announceEvent\(/);
  assert.match(consoleHtml, /function playConsoleBgm\(/);
});

test('Supabase 雲端成就徽章同步與一鍵登入', () => {
  const indexHtml = fs.readFileSync(path.join(root, 'index.html'), 'utf8');
  assert.match(indexHtml, /supabase-js/);
  assert.match(indexHtml, /id="cloud-sync-container"/);
  assert.match(game, /function initCloudSync\(/);
  assert.match(game, /initCloudSync\(\)/);
...
test('出招3D招式字卡與大絕招電影暗幕/震撼文字3D呈現', () => {
  const indexHtml = fs.readFileSync(path.join(root, 'index.html'), 'utf8');
  const styleCss = fs.readFileSync(path.join(root, 'style.css'), 'utf8');
  assert.match(indexHtml, /id="cinematic-overlay"/);
  assert.match(styleCss, /\.move-name-callout/);
  assert.match(styleCss, /\.ultimate-title-3d/);
...
test('怪物回合專屬警報與觀察視角 UIUX 區隔 (cpu-turn-mode)', () => {
  const styleCss = fs.readFileSync(path.join(root, 'style.css'), 'utf8');
  assert.match(styleCss, /\.command-deck\.cpu-turn-mode/);
  assert.match(styleCss, /\.question-card\.cpu-turn-mode/);
  assert.match(styleCss, /cpu-scanline-anim/);
